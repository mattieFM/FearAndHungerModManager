...
class Region {
	...
	split(minPercentageLeft = 20, percentagePadding = 10, forceHorizontal=undefined) {
		...
		const splitHorizontally = forceHorizontal==undefined ? Math.random() > .5 : forceHorizontal;
	...
	splitTillWidth(width, height) {
		let force = undefined;
		if(MATTIE.util.checkNumberInRange(this.getWidth(),0, width)) return; else force=0;
		if(MATTIE.util.checkNumberInRange(this.getHeight(),0, height)) return; else force=1;
		this.split(force);
		if (!this.checkLeaf()) {
			this.children.forEach(child => {
				//decrement by one and recurse
